...
void DatasetPlotter(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string xmlNom, string CraneenPath)
{
    ...
    for (int d = 0; d < datasets.size(); d++)  //Loop through datasets
    {
        ...
        if (dataSetName.find("ttbar_ll")!=string::npos ||dataSetName.find("ttbar_cc")!=string::npos ||dataSetName.find("ttbar_bb")!=string::npos){
                ...
                if (dataSetName == "TTJets"){
                    ...
                    if(split_ttbar){
                        if(ttbar_flav==0){//light
                            ...
                            MSPlot[plotname]->Fill(varofInterest, ttbar_bb, true, NormFactor*ScaleFactor*Luminosity);                
                        }          
                    }
                    else{
